// BiblioFa - script.js
// v1.0.8 - Tarjetas de libros rediseñadas (Géneros + Random)
...
// --- Tarjetas de libros (pestaña Géneros) ---
  ...
  data.forEach(libro => {
    ...
    div.innerHTML = `
      <div class="book-card-top">
        ${estrellas ? `<span class="book-card-stars">${"⭐".repeat(Number(estrellas))}</span>` : '<span></span>'}
        ${flags ? `<span class="book-card-flag">${escapeHtml(flags)}</span>` : ''}
      </div>
      <div class="book-card-body">
        <h3 class="book-card-title">${escapeHtml(titulo)}</h3>
        <p class="book-card-author">✍️ ${escapeHtml(autor)}</p>
        <span class="book-card-genre">${escapeHtml(genero)}</span>
      </div>
    `;
...
document.getElementById('btnRandom').addEventListener('click', ()=>{
  ...
  div.innerHTML = `
    <div class="random-card">
      <div class="book-card-top" style="padding:0;justify-content:center;gap:10px;">
        ${estrellas ? `<span class="book-card-stars">${"⭐".repeat(Number(estrellas))}</span>` : ''}
        ${flags ? `<span class="book-card-flag">${escapeHtml(flags)}</span>` : ''}
      </div>
